...
const LeetCodeStats = () => {
  ...
  const statItems = stats ? [
    { label: 'Total Solved', value: stats.totalSolved, color: '#ffa800', bg: 'rgba(0,0,0,0.02)', border: 'rgba(255,168,0,0.2)' },
    { label: 'Easy', value: stats.easySolved, color: '#00b8a3', bg: 'rgba(0,0,0,0.02)', border: 'rgba(0,184,163,0.2)' },
    { label: 'Medium', value: stats.mediumSolved, color: '#ffc01e', bg: 'rgba(0,0,0,0.02)', border: 'rgba(255,192,30,0.2)' },
    { label: 'Hard', value: stats.hardSolved, color: '#ef4743', bg: 'rgba(0,0,0,0.02)', border: 'rgba(239,71,67,0.2)' },
  ] : [];
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
          {statItems.map((item, i) => (
            <div key={i} style={{
              ...
              <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--muted-ink)', marginTop: '4px', letterSpacing: '0.04em', textTransform: 'uppercase' }}>
                {item.label}
              </div>
            </div>
          ))}
